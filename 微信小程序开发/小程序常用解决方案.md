...
```js
    let that = this;
    console.log('触发底部刷新');
    let localPage = that.data.page + 1; //获取当期的起始获取索引值
    wx.showLoading({
      title: '加载中...',
    })
    ...
    app.request('POST', '/api/xxx', data).then(res => {
      ...
      if (res.data == '' && res.data.length == 0) {
        ...
        let addNext = that.data.commList;
        addNextRes = res.data;
        for (let item in addNextRes) {
...
- 线上版本（手机调试）跟开发工具调试token不统一，经常报token无效或openid无效=》取消掉不校验合法域名和清除缓存数据、最好也清除掉使用的小程序记录数据；
...
//util.js下载接口：
// 保存二维码图片
const downloadActCodeImg = imgurl => {
  wx.downloadFile({
    url: imgurl,
    success: function(res) {
...
1.  安卓手机不支持console.dir()方法，会报错停止执行代码===》小程序使用console.dir();导致安卓程序停止，苹果无影响！，尽量不要使用console.dir();
...
```js
    ...
    qSearch.exec(function(res) {
      ...
      that.setData({
        useHeith: (parseInt(that.data.useHeith) + parseInt(res[0].height))
      })
      //timeAndTipsFixed高度
      ...
      let result = qKillOrder.exec(function(resk) {
        ...
        that.setData({
          useHeith: (parseInt(that.data.useHeith) + parseInt(resk[0].height))
        })
        // bannerType-view高度
        wx.getSystemInfo({
          success: function(resPhone) {
            ...
            that.setData({
              ...
              scrollViewHeith: (parseInt(resPhone.windowHeight) - parseInt(that.data.useHeith))
            })
